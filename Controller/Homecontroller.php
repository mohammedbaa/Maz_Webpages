<?php
  ...
  class Homecontroller extends Basecontroller
  {
    ...
    {
      $sql = "SELECT id,address,house_type,rental_value,apartment_type FROM tbl_house WHERE rental_value BETWEEN :min and :max and active_status = :active_status";
      ...
      if (isset($data['house_type'])) {
        ...
        if (isset($data['apartment_type'])) {
            $sql = $sql.' and apartment_type = :apartment_type';
            $apartment_type = 1;
        }
      ...
      if ($house_type == 1) {
        ...
        if ($apartment_type == 1) {
            $query->bindParam(':apartment_type', $data['apartment_type'], PDO::PARAM_STR);
        }
      $query->bindValue(':active_status',0);
      $query->execute();
      $result = $query->fetchAll();
    ...
    public function deletehouse($ids,$ownerid){
      ...
      $query->execute();
      Header("location:http://localhost/homeR/owner_profile.php?id=".$ownerid);
    }
